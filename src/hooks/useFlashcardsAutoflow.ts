...
const ENDGAME_LOWER_BOXES: readonly (keyof BoxesState)[] = [
  "boxOne",
  "boxTwo",
  "boxThree",
];
const ENDGAME_UPPER_BOXES_BY_PRIORITY: readonly (keyof BoxesState)[] = [
  "boxFive",
  "boxFour",
];
const ENDGAME_LOWER_SHARE_THRESHOLD = 0.1;
...
export type AutoflowPhase = "earlyGame" | "endGame";

type PickAutoflowDecisionParams = {
  boxes: BoxesState;
  activeBox: keyof BoxesState | null;
  boxZeroEnabled: boolean;
  ...
  remainingNewFlashcardsCount?: number;
  lockedEndGameDrainTarget?: keyof BoxesState | null;
};

export function resolveAutoflowPhase(params: {
  remainingNewFlashcardsCount?: number;
}): AutoflowPhase {
  return (params.remainingNewFlashcardsCount ?? 1) > 0
    ? "earlyGame"
    : "endGame";
}
...
function sumBoxes(
  boxes: BoxesState,
  boxList: readonly (keyof BoxesState)[]
): number {
  return boxList.reduce((sum, box) => sum + (boxes[box]?.length ?? 0), 0);
}

function shouldStartEndGameDrainSequence(boxes: BoxesState): boolean {
  const lowerTotal = sumBoxes(boxes, ENDGAME_LOWER_BOXES);
  const upperTotal = sumBoxes(boxes, ENDGAME_UPPER_BOXES_BY_PRIORITY);
  const relevantTotal = lowerTotal + upperTotal;

  if (relevantTotal <= 0 || upperTotal <= 0) {
    return false;
  }

  return lowerTotal / relevantTotal <= ENDGAME_LOWER_SHARE_THRESHOLD;
}

function resolveLockedEndGameDrainTarget(
  boxes: BoxesState,
  lockedTarget?: keyof BoxesState | null
): keyof BoxesState | null {
  if (lockedTarget === "boxFive") {
    if ((boxes.boxFive?.length ?? 0) > 0) {
      return "boxFive";
    }
    if ((boxes.boxFour?.length ?? 0) > 0) {
      return "boxFour";
    }
    return null;
  }

  if (lockedTarget === "boxFour") {
    return (boxes.boxFour?.length ?? 0) > 0 ? "boxFour" : null;
  }

  return null;
}

function pickInitialEndGameDrainTarget(
  boxes: BoxesState
): keyof BoxesState | null {
  return (
    ENDGAME_UPPER_BOXES_BY_PRIORITY.find((box) => (boxes[box]?.length ?? 0) > 0) ??
    null
  );
}

function pickSharedAutoflowDecision(
...
export function pickEarlyGameAutoflowDecision(
  params: PickAutoflowDecisionParams
): AutoflowDecision {
...
export function pickEndGameAutoflowDecision(
  ...
  const lockedTarget = resolveLockedEndGameDrainTarget(
    boxes,
    lockedEndGameDrainTarget
  );
  if (lockedTarget) {
    return { targetBox: lockedTarget, shouldDownloadNew: false };
  }

  if (shouldStartEndGameDrainSequence(boxes)) {
    const initialTarget = pickInitialEndGameDrainTarget(boxes);
    if (initialTarget) {
      return { targetBox: initialTarget, shouldDownloadNew: false };
    }
  }
...
export function pickAutoflowDecision(
  params: PickAutoflowDecisionParams
): AutoflowDecision {
  const phase = resolveAutoflowPhase({
    remainingNewFlashcardsCount: params.remainingNewFlashcardsCount,
  });

  if (phase === "endGame") {
    return pickEndGameAutoflowDecision(params);
  }

  return pickEarlyGameAutoflowDecision(params);
}
...
export function useFlashcardsAutoflow({
  ...
  const endGameDrainTargetRef = useRef<keyof BoxesState | null>(null);
  ...
  useEffect(() => {
    ...
    const phase = resolveAutoflowPhase({ remainingNewFlashcardsCount });

    if (phase !== "endGame") {
      endGameDrainTargetRef.current = null;
    } else if (!boxZeroEnabled || (boxes.boxZero?.length ?? 0) === 0) {
      const lockedTarget = resolveLockedEndGameDrainTarget(
        boxes,
        endGameDrainTargetRef.current
      );

      if (lockedTarget) {
        endGameDrainTargetRef.current = lockedTarget;
      } else if (shouldStartEndGameDrainSequence(boxes)) {
        endGameDrainTargetRef.current = pickInitialEndGameDrainTarget(boxes);
      } else {
        endGameDrainTargetRef.current = null;
      }
    }
    ...
    const decision = pickAutoflowDecision({
      ...
      flushThreshold,
      remainingNewFlashcardsCount,
      lockedEndGameDrainTarget: endGameDrainTargetRef.current,
    });

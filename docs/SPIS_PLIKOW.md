...
3. src/components/box/boxcarousel.tsx: Karuzela pudełek z interpolacją scrolla i wyborem aktywnego boxa. Używany: tak (src/screens/flashcards/FlashcardsScreen.tsx).
...
6. src/components/card/card.tsx: Formularz odpowiedzi dla fiszek z walidacją, korektą i sterowaniem fokusami. Używany: tak (src/screens/flashcards/FlashcardsScreen.tsx).
...
8. src/components/confetti/Confetti.tsx: Generuje animowane konfetti na potrzeby celebracji poprawnych odpowiedzi. Używany: tak (ekran fiszek: src/screens/flashcards/FlashcardsScreen.tsx).
...
6. src/screens/flashcards/FlashcardsScreen.tsx: Trening fiszek (kursy wbudowane i własne) z systemem pudełek SRS i celebracją postępów. Używany: tak (app/flashcards/index.tsx, app/flashcards_custom/index.tsx).
7. src/screens/home/HomeScreen.tsx: Powitalny ekran aplikacji z nawigacją do panelu kursów, statystyk i kreatora customowego. Używany: tak (app/index.tsx).
8. src/screens/legacy/createcourse/CreateCourseScreen.tsx: Historyczny ekran tworzenia kursu na bazie wyboru flag. Używany: nie (brak eksportu w katalogu app).
9. src/screens/legacy/review/ReviewLevelsScreen.tsx: Lista poziomów CEFR do powtórek z licznikiem zaległych kart, prowadzi do gry memory. Używany: tak (app/review/levels.tsx).
10. src/screens/review/courses/CoursesReviewScreen.tsx: Hub sesji powtórkowych dla kursów wbudowanych, oficjalnych i customowych. Używany: tak (app/review/index.tsx).
11. src/screens/review/memorygame/MemoryGameScreen.tsx: Gra memory łącząca pary słowo–tłumaczenie dla powtórek SRS. Używany: tak (app/review/memory.tsx).
12. src/screens/review/session/TypingReviewScreen.tsx: Sesja powtórek z wpisywaniem odpowiedzi i kolejką pytań opartą o RotaryStack. Używany: tak (app/review/session.tsx).
13. src/screens/review/temp/LegacyReviewScreen.tsx: Tymczasowy wariant widoku wyboru poziomu do powtórek tekstowych. Używany: nie (nie ma trasy w katalogu app).
14. src/screens/settings/SettingsScreen.tsx: Zakładkowy ekran ustawień (wygląd, nauka, dostępność, dane). Używany: tak (app/settings/index.tsx).
15. src/screens/stats/StatsScreen.tsx: Dashboard statystyk z heatmapą aktywności, progresami kursów i listami słówek. Używany: tak (app/stats/index.tsx).

# Hooki
1. src/hooks/useBoxesPersistenceSnapshot.ts: Utrzymuje stan pudełek SRS w AsyncStorage, raportuje postęp i zarządza kolejką słówek. Używany: tak (src/screens/flashcards/FlashcardsScreen.tsx).
...
3. src/hooks/useFlashcardsInteraction.ts: Zarządza logiką nauki (wybór pudełka, sprawdzanie odpowiedzi, logowanie postępów, promocja kart). Używany: tak (src/screens/flashcards/FlashcardsScreen.tsx).
...
5. src/hooks/useSpellchecking.ts: Zwraca funkcję sprawdzającą odpowiedzi z tolerancją literówek i obsługą diakrytyków. Używany: tak (src/screens/flashcards/FlashcardsScreen.tsx, src/screens/review/session/TypingReviewScreen.tsx).
...
11. app/flashcards/index.tsx: Trasa do głównego treningu fiszek dla kursów wbudowanych. Używany: tak (nawigacja z panelu kursów i HomeScreen).
...
14. app/profilpanel/index.tsx: Alias do CourseActivateScreen pod dawnym adresem profilpanel. Używany: nie (brak odwołań).
15. app/review/index.tsx: Hub powtórek (CoursesReviewScreen). Używany: tak (nawigacja z Navbar).
16. app/review/levels.tsx: Alias do Legacy ReviewLevelsScreen. Używany: nie (niewywoływany w nawigacji).
17. app/review/memory.tsx: Trasa gry memory wykorzystywanej w powtórkach. Używany: tak (wywołania z hubu powtórek).
18. app/review/session.tsx: Trasa sesji pisemnych powtórek TypingReviewScreen. Używany: tak (CoursesReviewScreen, LegacyReviewScreen).
19. app/settings/index.tsx: Trasa do ekranów ustawień. Używany: tak (np. skrót z Navbar).
20. app/stats/index.tsx: Trasa do dashboardu statystyk. Używany: tak (przycisk na HomeScreen i Navbar).
21. app.json: Konfiguracja Expo (nazwa aplikacji, uprawnienia, ikony). Używany: tak (w procesie build/run).
22. package.json: Manifest npm z zależnościami i skryptami. Używany: tak (zarządzanie projektem).
23. tsconfig.json: Konfiguracja TypeScript (aliasy, opcje kompilatora). Używany: tak (kompilacja/IDE).
24. metro.config.js: Konfiguracja bundlera Metro dla Expo. Używany: tak (podczas bundlowania).
25. README.md: Opis projektu i instrukcje uruchomienia. Używany: tak (dokumentacja).
26. src/config/appConfig.ts: Stałe konfiguracyjne aplikacji (np. batch fiszek). Używany: tak (importy m.in. w FlashcardsScreen).
27. src/constants/achievements.ts: Definicje odznak i progów statystyk. Używany: tak (LearningStatsContext, AchievementsList).
28. src/constants/customCourse.ts: Dane i helpery kursów customowych (ikony, kolory). Używany: tak (CustomCourseForm, CourseIconColorSelector).
29. src/constants/languageFlags.ts: Mapa flag językowych i helper getFlagSource. Używany: tak (CourseActivateScreen, CoursePinScreen itd.).
30. src/constants/memoryGame.ts: Konfiguracja planszy memory i etykiety rozmiarów. Używany: tak (MemoryGameScreen, Settings).
31. src/constants/officialPacks.ts: Lista oficjalnych paczek kursów. Używany: tak (CourseActivateScreen, CoursePinScreen, CoursesReviewScreen).
32. src/contexts/LearningStatsContext.tsx: Kontekst postępów użytkownika (poznane słowa, odznaki). Używany: tak (wrapowany w app/_layout.tsx).
33. src/contexts/PopupContext.tsx: Kontekst odpowiedzialny za popup Boxika. Używany: tak (provider w app/_layout.tsx, hook usePopup).
34. src/contexts/SettingsContext.tsx: Główny kontekst ustawień i preferencji. Używany: tak (provider w app/_layout.tsx).
35. src/db/schema/schema.sql: Pełny schemat bazy SQLite do referencji. Używany: pośrednio (materiał źródłowy dla narzędzi/db).
36. src/db/sqlite/db.ts: Warstwa dostępu do bazy SQLite (query runtime). Używany: tak (liczne ekrany i hooki).
37. src/db/sqlite/dbGenerator.ts: Generator danych testowych (batch słówek). Używany: tak (FlashcardsScreen).
38. src/features/customCourse/manualCards/ManualCardsEditor.tsx: Komponent edycji fiszek customowych. Używany: tak (CustomCourseContentScreen, EditCustomCourseScreen).
39. src/features/customCourse/manualCards/useManualCardsForm.ts: Hook formularza fiszek (historia, walidacje). Używany: tak (CustomCourseContentScreen, EditCustomCourseScreen).
40. src/theme/createThemeStylesHook.ts: Fabryka hooków styli zależnych od motywu. Używany: tak (większość plików *-styles.ts).
41. src/theme/theme.ts: Definicja palety i typów motywu. Używany: tak (SettingsContext, komponenty styli).
42. src/types/boxes.ts: Typy danych pudełek i fiszek. Używany: tak (hooki SRS, komponenty).
43. src/types/course.ts: Typy kursów językowych/customowych. Używany: tak (ekrany i konteksty).
44. src/types/language.ts: Typy związane z językami i poziomami CEFR. Używany: tak (Settings).
45. src/utils/diacritics.ts: Helper usuwający diakrytyki przy porównywaniu tekstu. Używany: tak (useSpellchecking, Card).
46. src/utils/flashcardsStorage.ts: Util do czyszczenia zapisanych pudełek w AsyncStorage. Używany: nie (tylko opis w dokumentacji).
